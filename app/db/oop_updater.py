...
class UpdateLeaguesTask(Task):
    ...
    def execute(self, **kwargs) -> None:
        ...
        self.loader.ensure_table_and_columns(
            'leagues',
            cleaned_leagues[0] if cleaned_leagues else {}
            )
...
class UpdateCountriesTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        self.loader.ensure_table_and_columns(
            'countries',
            countries_data[0] if countries_data else {}
            )
...
class UpdateMatchesTask(Task):
    ...
    def register_arguments(cls, parser: argparse.ArgumentParser):
        parser.add_argument(
            "--date",
            type=str,
            help="[Matches] Specify a date in YYYY-MM-DD format."
            )
    ...
    def execute(self, **kwargs):
        ...
        self.loader.ensure_table_and_columns(
            'matches',
            matches_data[0] if matches_data else {}
            )
...
class UpdateLeagueStatsTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        league_stats_data = self.client.get_league_stats(
            season_id=season_id,
            max_time=max_time
            )
        ...
        self.loader.ensure_table_and_columns(
            'league_stats',
            league_stats_data[0] if league_stats_data else {}
            )
...
class UpdateSchedulesTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        schedules_data = self.client.get_schedule(
            season_id=season_id,
            max_time=max_time
            )
        ...
        self.loader.ensure_table_and_columns(
            'matches',
            schedules_data[0] if schedules_data else {}
            )
...
class UpdateTeamsTask(Task):
    ...
    def register_arguments(cls, parser: argparse.ArgumentParser):
        parser.add_argument(
            "--stats",
            action="store_true",
            help="[Teams] Include detailed stats when fetching teams."
            )
    ...
    def execute(self, **kwargs):
        ...
        teams_data = self.client.get_league_teams(
            season_id=season_id,
            stats=stats,
            max_time=max_time
            )
        ...
        # If stats is selected, teams data contains nested dict which is a 
        # problem for flat table
        cleaned_teams = []
        ...
        self.loader.ensure_table_and_columns(
            'teams',
            cleaned_teams[0] if cleaned_teams else {}
            )
...
class UpdatePlayersTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        players_data = self.client.get_league_players(
            season_id=season_id,
            max_time=max_time
            )
        ...
        self.loader.ensure_table_and_columns(
            'players',
            players_data[0] if players_data else {}
            )
...
class UpdateRefereesTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        self.loader.ensure_table_and_columns(
            'referees',
            referees_data[0] if referees_data else {}
            )
...
class UpdateMatchDetailsTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        self.loader.ensure_table_and_columns(
            'match_details',
            match_details_data[0] if match_details_data else {}
            )
...
class UpdateLeagueTableTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        league_table_data = self.client.get_league_table(
            season_id=season_id,
            max_time=max_time
            )
        ...
        self.loader.ensure_table_and_columns(
            'league_table',
            league_table_data[0] if league_table_data else {}
            )
...
class UpdatePlayerStatsTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        self.loader.ensure_table_and_columns(
            'players',
            player_stats_data[0] if player_stats_data else {}
            )
...
class UpdateRefereeStatsTask(Task):
    ...
    def execute(self, **kwargs):
        ...
        referee_stats_data = self.client.get_referee_stats(
        ...
        self.loader.ensure_table_and_columns(
            'referees',
            referee_stats_data[0] if referee_stats_data else {}
            )
    ...
    def _get_ids_from_table(self, table_name: str, column_name: str) -> list:
        ...
        try:
            ...
            if cursor.fetchone() is None:
                print(
                    f"Warning: Table '{table_name}' not found for querying. \
    ...
    def execute(self, **kwargs):
        ...
        def run_task(task_name, **params):
            if task_name in task_instances:
                ...
                print(
                    f"Warning: Orchestrator tried to run unregistered task '{task_name}'."
                    )
        ...
        for season_id in season_ids:
            for task_name in season_tasks:
                run_task(task_name, season_id=season_id, stats=True)
...
class DatabaseUpdater:
        ...
        try:
            for task_name in task_names:
                if task_name not in self.registered_tasks:
                    print(
                        f"Warning: Task '{task_name}' is not registered. \
                        Skipping."
                        )
                    continue
                task_class = self.registered_tasks[task_name]
                task_instance = task_class(self.client, self.loader)
                # Pass the full registry to the comprehensive task so it can
                # create other tasks
    ...
    def _create_parser(self):
        ...
        parser.add_argument(
            ...
            type=str,
            nargs='+',
            choices=list(self.registered_tasks.keys()),
            metavar='TASK',
            help="Specify one or more tasks to run."
            )
        parser.add_argument(
            "--all",
            action="store_true",
            help="Run a comprehensive, cascading update of the entire database."
            )
        ...
        parser.add_argument(
            "--season_id",
            type=int,
            help="ID for a specific season."
            )
        parser.add_argument(
            "--max_time",
            type=int,
            help="Unix timestamp for ending point of data."
            )
        parser.add_argument(
            "--team_id",
            type=int,
            help="ID for a specific team."
            )
        parser.add_argument(
            "--match_id",
            type=int,
            help="ID for a specific match."
            )
        parser.add_argument(
            "--player_id",
            type=int,
            help="ID for a specific player."
            )
        parser.add_argument(
            "--referee_id",
            type=int,
            help="ID for a specific referee."
            )
